...
using Object = UnityEngine.Object;
    ...
    public static string AtlasName = "Atlas"; 
     public static string NewAtlasPath = "/Bundles/Atlas/";
    ...
    {
        //将Res/UI目录下的Atlas 打成 图集
        string uiPath = Path.Combine(Application.dataPath+"/Res/", dir);
       // Debug.LogError($"UIPath--------"+uiPath);
        ...
        foreach (DirectoryInfo dirInfo in uiDirInfo.GetDirectories())
        {
            ...
            {
               // Debug.LogError($"seconddirInfo.Name--------"+seconddirInfo.Name);
                if (seconddirInfo.Name.Contains(AtlasName))
                {
                    AtlasName = seconddirInfo.Name;
                    hasAtlas = true;
                }
    ...
    private static void CreateAtlasByFolders(DirectoryInfo dirInfo, DirectoryInfo atlasDir)
    {
       ...
       // Debug.LogError("assetPath-----"+assetPath);
        //string creatPath = Application.dataPath + NewAtlasPath;
        SetSpriteAtlas(sptAtlas, assetPath);
    ...
    {
        fullName = Application.dataPath + NewAtlasPath;
    ...
    public static void ClearAllAtlas()
    {
        ...
        string uiPath = Path.Combine(Application.dataPath, "Bundles", "Atlas");
       // Debug.Log(uiPath);
    ...
    public static void SettingPNG()
    {
        /*    string[] fileStrs = Directory.GetFiles(Path.GetFullPath("Assets/AssetsPackage/UI"), "*.*", SearchOption.AllDirectories);
            ...
            */
        string[] paths = Directory.GetFileSystemEntries(Path.GetFullPath("Assets/Res"));
        for (int i = 0; i < paths.Length; i++)
        {
            if (Directory.Exists(paths[i]))
            {
                ...
                string[] fileStrs = Directory.GetFiles(Path.GetFullPath("Assets/Res/" + di.Name), "*.*",
